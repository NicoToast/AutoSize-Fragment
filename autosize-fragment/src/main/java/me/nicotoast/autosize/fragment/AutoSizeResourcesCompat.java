...
/**
 * 拷贝自{@link me.jessyan.autosize.AutoSizeCompat} 删除了 Application.getResources() 的适配
 * ================================================ 当遇到本来适配正常的布局突然出现适配失效，适配异常等问题, 重写当前
 * {@link Activity} 的 {@link Activity#getResources()} 并调用 {@link AutoSizeResourcesCompat}
 * 的对应方法即可解决问题
 * <p>
 ...
 * ================================================
 */
public final class AutoSizeResourcesCompat {
  ...
  private AutoSizeResourcesCompat() {
   ...
   *
   * @param resources {@link Resources}
   * @param externalAdaptInfo 三方库的 {@link Activity} 或 Fragment 提供的适配参数, 需要配合
   ...
   * {@link DisplayMetrics#xdpi} 以支持单位 {@code pt}、{@code in}、{@code mm}
   *
   * @param resources {@link Resources}
  ...
  public static void autoConvertDensity(Resources resources, float sizeInDp,
    ...
    float subunitsDesignSize =
        isBaseOnWidth ? AutoSizeConfig.getInstance().getUnitsManager().getDesignWidth()
    ...
    int key = Math.round((sizeInDp + subunitsDesignSize + screenSize) * AutoSizeConfig.getInstance()
        .getInitScaledDensity()) & ~MODE_MASK;
    key = isBaseOnWidth ? (key | MODE_ON_WIDTH) : (key & ~MODE_ON_WIDTH);
    key = AutoSizeConfig.getInstance().isUseDeviceSize() ? (key | MODE_DEVICE_SIZE)
        : (key & ~MODE_DEVICE_SIZE);
    ...
    if (displayMetricsInfo == null) {
      ...
      if (AutoSizeConfig.getInstance().getPrivateFontScale() > 0) {
        ...
        float systemFontScale =
            AutoSizeConfig.getInstance().isExcludeFontScale() ? 1 : AutoSizeConfig.getInstance().
    ...
    AutoSizeLog.d(String.format(Locale.ENGLISH,
        ...
        resources.getClass().getName(), resources.getClass().getSimpleName(), isBaseOnWidth,
        isBaseOnWidth ? "designWidthInDp" : "designHeightInDp", sizeInDp,
        isBaseOnWidth ? "designWidthInSubunits" : "designHeightInSubunits", subunitsDesignSize,
        targetDensity, targetScaledDensity, targetDensityDpi, targetXdpi, targetScreenWidthDp,
...
//    }
  ...
  private static void setScreenSizeDp(Resources resources, int screenWidthDp, int screenHeightDp) {
    if (AutoSizeConfig.getInstance().getUnitsManager().isSupportDP() && AutoSizeConfig.getInstance()
        .getUnitsManager().isSupportScreenSizeDP()) {
